...
def get_pretrained_model(IMG_SIZE):
...
def compile_train_model(base_model, train_df, val_df, ds_train, ds_val, early_stopping, plateau):
    keras.backend.clear_session()
    model_pretrained = get_pretrained()
    model_pretrained.compile(loss='binary_crossentropy'
                  , optimizer = keras.optimizers.Adam(learning_rate=5e-5), metrics='binary_accuracy')
    model_pretrained.summary()
    model_pretrained.fit(ds_train,
          batch_size = BATCH, epochs = 10,
          validation_data=ds_val,
          callbacks=[early_stopping, plateau],
          steps_per_epoch=(len(train_df)/BATCH),
          validation_steps=(len(val_df)/BATCH));
    model_pretrained.save("xray_image_classification_resnet")
    return base_model, model_pretrained



def fine_tune(base_model, model_pretrained, ds_train, ds_val, train_df, val_df, early_stopping, plateau):
    ...
    model_pretrained.fit(ds_train,
          ...
          steps_per_epoch=(len(train_df)/BATCH),
          validation_steps=(len(val_df)/BATCH));
    model_pretrained.save("xray_image_classification_resnet_fine_tuned")
...
if __name__ == "__main__":
    ...
    model, base_model = get_pretrained_model(IMG_SIZE)
    base_model, model_pretrained = compile_train_model(base_model, 
                                                       train_df, 
                                                       val_df, 
                                                       ds_train, 
                                                       ds_val, 
                                                       early_stopping, 
                                                       plateau)
    model_pretrained = fine_tune(base_model, 
                                 model_pretrained, 
                                 ds_train, 
                                 ds_val, 
                                 train_df, 
                                 ...
                                 early_stopping, 
                                 plateau)
    val_loss, val_accuracy, test_loss, test_accuracy, confusion_matrix, clf_report = evaluate_model(model_pretrained, 
                                                                                                    ds_val, 
                                                                                                    ds_test, 
                                                                                                    val_df, 
                                                                                                    df_test)
